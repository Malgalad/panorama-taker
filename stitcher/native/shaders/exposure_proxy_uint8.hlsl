...
Buffer<uint> sources : register(t0);
RWStructuredBuffer<float3> proxies : register(u0);

[numthreads(64, 1, 1)]
void build_uint8_exposure_proxies(uint3 thread_id : SV_DispatchThreadID)
{
    ...
    for (uint sample_y = (uint)floor(top); sample_y < min((uint)ceil(bottom), source_height); ++sample_y)
    {
        ...
        {
            const float weight_x = max(0.0f, min(right, sample_x + 1.0f) - max(left, (float)sample_x));
            const uint source = frame * source_frame_offset_elements +
                sample_y * source_row_stride_elements + sample_x * 3u;
            const float weight = weight_x * weight_y;
            float3 value = float3(sources[source], sources[source + 1], sources[source + 2]) / 255.0f;
            if (transfer_function == 1u)
                value = value <= 0.04045f ? value / 12.92f : pow(max((value + 0.055f) / 1.055f, 0.0f), 2.4f);
