...
{
    const uint offset = source_frame_offset_elements + coordinate.y * source_row_stride_elements + coordinate.x * 3u;
    return decode_source_transfer(
        float3(source_values[offset], source_values[offset + 1u], source_values[offset + 2u]),
        transfer_function);
}

[numthreads(8, 8, 1)]
void generate_float32_candidate(uint3 thread_id : SV_DispatchThreadID)
{
