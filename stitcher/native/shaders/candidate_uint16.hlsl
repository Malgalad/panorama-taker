...
float3 load_source_rgb(const uint2 coordinate)
{
    ...
    const float3 encoded = float3(source_values[offset], source_values[offset + 1u], source_values[offset + 2u]) * (1.0f / 65535.0f);
    return decode_source_transfer(encoded, transfer_function);
}

[numthreads(8, 8, 1)]
void generate_uint16_candidate(uint3 thread_id : SV_DispatchThreadID)
{
