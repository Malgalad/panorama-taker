...
Buffer<float> source_values : register(t0);
...
{
    const uint source_offset = source_frame_offset_elements + coordinate.y * source_row_stride_elements + coordinate.x * 3u;
    return float3(
        source_values[source_offset], source_values[source_offset + 1u], source_values[source_offset + 2u]);
}

[numthreads(64, 1, 1)]
void sample_float32(uint3 thread_id : SV_DispatchThreadID)
{
    const uint index = thread_id.x;
    if (index >= coordinates.Length)
        return;
    const float2 coordinate = clamp(
        coordinates[index], float2(0.0f, 0.0f), float2(source_width - 1u, source_height - 1u));
